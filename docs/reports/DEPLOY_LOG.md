...
| --- | --- | --- | --- | --- | --- |
| 2026-09-14 | `24ef11c` | Git auto-deploy (see Deployments tab) | `.gitignore` only — no app code change. Added `docs/reports/PROJECT_STATUS_REPORT.md` and `docs/reports/project-status-artifact.html` to ignored files (ad-hoc project status report, kept local, not part of the `docs/reports/` per-change convention). | none | ✅ polled `GET /api/health` every 5s for ~40s post-push, stayed `200 ok` throughout. |
| 2026-09-14 | `6392b98` (merge of `fix/sidebar-signout-button-contrast`, `main`) | Git auto-deploy (see Deployments tab) | Fix: Sign out button in the sidebar had invisible (near-white-on-near-white) text in light mode — the `outline` Button variant never set its own text color and silently inherited `text-sidebar-foreground` from the sidebar, which stays dark-violet in both themes. Added `text-foreground` so it matches the button's own `bg-background` in both themes. | none (frontend only) | ✅ `npm run predeploy` green before push; polled `GET /api/health` every 5s for 1 minute post-push, stayed `200 ok` throughout the deploy window. |
